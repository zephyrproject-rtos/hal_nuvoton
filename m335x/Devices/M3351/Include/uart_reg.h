...
typedef struct
{
     ...
     * |        |          |This bit is set when LIN slave header detect (SLVHDETF (UART_LINSTS[0] = 1)), LIN slave header error detect (SLVHEF (UART_LINSTS[1] = 1)), LIN slave ID parity error (SLVIDPEF (UART_LINSTS[2] = 1)), LIN slave header time-out detect (SLVHTOF (UART_LINSTS[4] = 1)), LIN response time-out detect (RTOUTF (UART_LINSTS[5] = 1)), LIN break detect (BRKDETF (UART_LINSTS[8] = 1)), or bit error detect (BITEF (UART_LINSTS[9] = 1))
     ...
     * |        |          |Note: This bit is cleared when SLVHDETF (UART_LINSTS[0]), SLVHEF (UART_LINSTS[1]), SLVIDPEF (UART_LINSTS[2]), SLVHTOF (UART_LINSTS[4]), RTOUTF (UART_LINSTS[5]), BRKDETF (UART_LINSTS[8]), and BITEF (UART_LINSTS[9]) all are cleared and software writing '1' to LINIF (UART_INTSTS[7]).
     ...
     * |        |          |Note 4: The minimum detection range of the target BRD (UART_BAUD[15:0]) is 0x30.
     ...
     * |        |          |Note 1: If LINWKEN (UART_LINWKCTL[28]) is enabled, the LIN wake-up event will cause this bit set to '1'.
     * |        |          |Note 2: This bit can be cleared by writing '1' to it.
     */
    ...
    __IO uint32_t LINRTOUT;              /*!< [0x0050] UART LIN Response Time-out Register                              */
    __IO uint32_t LINWKCTL;              /*!< [0x0054] UART LIN Wake-up Control Register                                */
