...
typedef struct
{
     ...
     * |        |          |When read action is finished and the next action is going to read, R2R idle cycle is inserted and EBI_nCS return to idle state.
     * @var EBI_T::VERSION
     * Offset: 0xFFC  EBI RTL Design Version Number
     * ---------------------------------------------------------------------------------------------------
     * |Bits    |Field     |Descriptions
     * | :----: | :----:   | :---- |
     * |[15:0]  |MINOR     |EBI RTL Design MINOR Version Number
     * |        |          |Minor version number is dependent on ECO version control
     * |        |          |0x0000: (current Minor Version Number)
     * |[23:16] |SUB       |EBI RTL Design MINOR Version Number
     * |        |          |Sub version number is relative to key feature
     * |        |          |0x02: (current Sub Version Number)
     * |[31:24] |MAJOR     |EBI RTL Design MAJOR Version Number
    ...
    __I  uint32_t RESERVE2[1013];
    __I  uint32_t VERSION;               /*!< [0x0ffc] EBI RTL Design Version Number                                    */
