 ...
 * @file     hsusbd.h
 * @version  V3.00
 * @brief    High Speed USB 2.0 Device(HSUSBD) driver header file
 *
...
#define HSUSBD_ENABLE_USB()               (((__PC() & NS_OFFSET) == NS_OFFSET)? ((uint32_t)(HSUSBD_NS->PHYCTL |= (HSUSBD_PHYCTL_PHYEN_Msk|HSUSBD_PHYCTL_DPPUEN_Msk))):((uint32_t)(HSUSBD->PHYCTL |= (HSUSBD_PHYCTL_PHYEN_Msk|HSUSBD_PHYCTL_DPPUEN_Msk)))) /*!<Enable USB  \hideinitializer */
#define HSUSBD_DISABLE_USB()              (((__PC() & NS_OFFSET) == NS_OFFSET)? ((uint32_t)(HSUSBD_NS->PHYCTL &= ~HSUSBD_PHYCTL_DPPUEN_Msk)):((uint32_t)(HSUSBD->PHYCTL &= ~HSUSBD_PHYCTL_DPPUEN_Msk))) /*!<Disable USB  \hideinitializer */
#define HSUSBD_ENABLE_PHY()               (((__PC() & NS_OFFSET) == NS_OFFSET)? ((uint32_t)(HSUSBD_NS->PHYCTL |= HSUSBD_PHYCTL_PHYEN_Msk)):((uint32_t)(HSUSBD->PHYCTL |= HSUSBD_PHYCTL_PHYEN_Msk))) /*!<Enable PHY  \hideinitializer */
#define HSUSBD_DISABLE_PHY()              (((__PC() & NS_OFFSET) == NS_OFFSET)? ((uint32_t)(HSUSBD_NS->PHYCTL &= ~HSUSBD_PHYCTL_PHYEN_Msk)):((uint32_t)(HSUSBD->PHYCTL &= ~HSUSBD_PHYCTL_PHYEN_Msk))) /*!<Disable PHY  \hideinitializer */
#define HSUSBD_SET_SE0()                  (((__PC() & NS_OFFSET) == NS_OFFSET)? ((uint32_t)(HSUSBD_NS->PHYCTL &= ~HSUSBD_PHYCTL_DPPUEN_Msk)):((uint32_t)(HSUSBD->PHYCTL &= ~HSUSBD_PHYCTL_DPPUEN_Msk))) /*!<Enable SE0, Force USB PHY Transceiver to Drive SE0  \hideinitializer */
#define HSUSBD_CLR_SE0()                  (((__PC() & NS_OFFSET) == NS_OFFSET)? ((uint32_t)(HSUSBD_NS->PHYCTL |= HSUSBD_PHYCTL_DPPUEN_Msk)):((uint32_t)(HSUSBD->PHYCTL |= HSUSBD_PHYCTL_DPPUEN_Msk))) /*!<Disable SE0  \hideinitializer */
#define HSUSBD_SET_ADDR(addr)             (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->FADDR = (addr)):(HSUSBD->FADDR = (addr))) /*!<Set USB address  \hideinitializer */
#define HSUSBD_GET_ADDR()                 (((__PC() & NS_OFFSET) == NS_OFFSET)? ((uint32_t)(HSUSBD_NS->FADDR)):((uint32_t)(HSUSBD->FADDR))) /*!<Get USB address  \hideinitializer */
#define HSUSBD_ENABLE_USB_INT(intr)       (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->GINTEN = (intr)):(HSUSBD->GINTEN = (intr))) /*!<Enable USB Interrupt  \hideinitializer */
#define HSUSBD_ENABLE_BUS_INT(intr)       (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->BUSINTEN = (intr)):(HSUSBD->BUSINTEN = (intr))) /*!<Enable BUS Interrupt  \hideinitializer */
#define HSUSBD_GET_BUS_INT_FLAG()         (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->BUSINTSTS):(HSUSBD->BUSINTSTS))        /*!<Get Bus interrupt flag  \hideinitializer */
#define HSUSBD_CLR_BUS_INT_FLAG(flag)     (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->BUSINTSTS = (flag)):(HSUSBD->BUSINTSTS = (flag))) /*!<Clear Bus interrupt flag  \hideinitializer */
#define HSUSBD_ENABLE_CEP_INT(intr)       (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->CEPINTEN = (intr)):(HSUSBD->CEPINTEN = (intr))) /*!<Enable CEP Interrupt  \hideinitializer */
#define HSUSBD_CLR_CEP_INT_FLAG(flag)     (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->CEPINTSTS = (flag)):(HSUSBD->CEPINTSTS = (flag))) /*!<Clear CEP interrupt flag  \hideinitializer */
#define HSUSBD_SET_CEP_STATE(flag)        (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->CEPCTL = (flag)):(HSUSBD->CEPCTL = (flag))) /*!<Set CEP state  \hideinitializer */
#define HSUSBD_START_CEP_IN(size)         (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->CEPTXCNT = (size)):(HSUSBD->CEPTXCNT = (size))) /*!<Start CEP IN Transfer  \hideinitializer */
#define HSUSBD_SET_MAX_PAYLOAD(ep, size)  (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->EP[(ep)].EPMPS = (size)):(HSUSBD->EP[(ep)].EPMPS = (size))) /*!<Set EPx Maximum Packet Size  \hideinitializer */
#define HSUSBD_ENABLE_EP_INT(ep, intr)    (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->EP[(ep)].EPINTEN = (intr)):(HSUSBD->EP[(ep)].EPINTEN = (intr))) /*!<Enable EPx Interrupt  \hideinitializer */
#define HSUSBD_GET_EP_INT_FLAG(ep)        (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->EP[(ep)].EPINTSTS):(HSUSBD->EP[(ep)].EPINTSTS)) /*!<Get EPx interrupt flag  \hideinitializer */
#define HSUSBD_CLR_EP_INT_FLAG(ep, flag)  (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->EP[(ep)].EPINTSTS = (flag)):(HSUSBD->EP[(ep)].EPINTSTS = (flag))) /*!<Clear EPx interrupt flag  \hideinitializer */
#define HSUSBD_SET_DMA_LEN(len)           (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->DMACNT = (len)):(HSUSBD->DMACNT = (len))) /*!<Set DMA transfer length  \hideinitializer */
#define HSUSBD_SET_DMA_ADDR(addr)         (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->DMAADDR = (addr)):(HSUSBD->DMAADDR = (addr))) /*!<Set DMA transfer address  \hideinitializer */
#define HSUSBD_SET_DMA_READ(epnum)        (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->DMACTL = (HSUSBD_NS->DMACTL & ~HSUSBD_DMACTL_EPNUM_Msk) | HSUSBD_DMACTL_DMARD_Msk | (epnum) | 0x100):(HSUSBD->DMACTL = (HSUSBD->DMACTL & ~HSUSBD_DMACTL_EPNUM_Msk) | HSUSBD_DMACTL_DMARD_Msk | (epnum) | 0x100)) /*!<Set DMA transfer type to read \hideinitializer */
#define HSUSBD_SET_DMA_WRITE(epnum)       (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->DMACTL = (HSUSBD_NS->DMACTL & ~(HSUSBD_DMACTL_EPNUM_Msk | HSUSBD_DMACTL_DMARD_Msk | 0x100)) | (epnum)):(HSUSBD->DMACTL = (HSUSBD->DMACTL & ~(HSUSBD_DMACTL_EPNUM_Msk | HSUSBD_DMACTL_DMARD_Msk | 0x100)) | (epnum))) /*!<Set DMA transfer type to write \hideinitializer */
#define HSUSBD_ENABLE_DMA()               (((__PC() & NS_OFFSET) == NS_OFFSET)? (HSUSBD_NS->DMACTL |= HSUSBD_DMACTL_DMAEN_Msk):(HSUSBD->DMACTL |= HSUSBD_DMACTL_DMAEN_Msk)) /*!<Enable DMA transfer  \hideinitializer */
#define HSUSBD_IS_ATTACHED()              (((__PC() & NS_OFFSET) == NS_OFFSET)? ((uint32_t)(HSUSBD_NS->PHYCTL & HSUSBD_PHYCTL_VBUSDET_Msk)):((uint32_t)(HSUSBD->PHYCTL & HSUSBD_PHYCTL_VBUSDET_Msk))) /*!<Check cable connect state  \hideinitializer */
#define HSUSBD_ENABLE_BCD()               (((__PC() & NS_OFFSET) == NS_OFFSET)? ((uint32_t)(HSUSBD_NS->BCDC |= HSUSBD_BCDC_BCDEN_Msk)):((uint32_t)(HSUSBD->BCDC |= HSUSBD_BCDC_BCDEN_Msk))) /*!<Enable BCD  \hideinitializer */
#define HSUSBD_DISABLE_BCD()              (((__PC() & NS_OFFSET) == NS_OFFSET)? ((uint32_t)(HSUSBD_NS->BCDC &= ~HSUSBD_BCDC_BCDEN_Msk)):((uint32_t)(HSUSBD->BCDC &= ~HSUSBD_BCDC_BCDEN_Msk))) /*!<Disable BCD  \hideinitializer */
#define HSUSBD_ENABLE_LPM()               (((__PC() & NS_OFFSET) == NS_OFFSET)? ((uint32_t)(HSUSBD_NS->LPMCSR |= HSUSBD_LPMCSR_LPMEN_Msk)):((uint32_t)(HSUSBD->LPMCSR |= HSUSBD_LPMCSR_LPMEN_Msk))) /*!<Enable LPM  \hideinitializer */
#define HSUSBD_DISABLE_LPM()              (((__PC() & NS_OFFSET) == NS_OFFSET)? ((uint32_t)(HSUSBD_NS->LPMCSR &= ~HSUSBD_LPMCSR_LPMEN_Msk)):((uint32_t)(HSUSBD->LPMCSR &= ~HSUSBD_LPMCSR_LPMEN_Msk))) /*!<Disable LPM  \hideinitializer */
#define HSUSBD_ENABLE_LPMSLEEP()          (((__PC() & NS_OFFSET) == NS_OFFSET)? ((uint32_t)(HSUSBD_NS->LPMCSR |= HSUSBD_LPMCSR_LPMSLEEPEN_Msk)):((uint32_t)(HSUSBD->LPMCSR |= HSUSBD_LPMCSR_LPMSLEEPEN_Msk))) /*!<Enable LPMSLEEP  \hideinitializer */
#define HSUSBD_DISABLE_LPMSLEEP()         (((__PC() & NS_OFFSET) == NS_OFFSET)? ((uint32_t)(HSUSBD_NS->LPMCSR &= ~HSUSBD_LPMCSR_LPMSLEEPEN_Msk)):((uint32_t)(HSUSBD->LPMCSR &= ~HSUSBD_LPMCSR_LPMSLEEPEN_Msk))) /*!<Disable LPMSLEEP  \hideinitializer */
#define HSUSBD_ENABLE_HS_HANDSHAKE()      (((__PC() & NS_OFFSET) == NS_OFFSET)? ((uint32_t)(HSUSBD_NS->OPER |= HSUSBD_OPER_HISHSEN_Msk)):((uint32_t)(HSUSBD->OPER |= HSUSBD_OPER_HISHSEN_Msk))) /*!<Enable HISHSEN  \hideinitializer */
#define HSUSBD_DISABLE_HS_HANDSHAKE()     (((__PC() & NS_OFFSET) == NS_OFFSET)? ((uint32_t)(HSUSBD_NS->OPER &= ~HSUSBD_OPER_HISHSEN_Msk)):((uint32_t)(HSUSBD->OPER &= ~HSUSBD_OPER_HISHSEN_Msk))) /*!<Disable HISHSEN  \hideinitializer */
...
__STATIC_INLINE void HSUSBD_ResetDMA(void)
{
    ...
    pHSUSBD->DMACNT = 0ul;
    pHSUSBD->DMACTL = 0x80ul;
    pHSUSBD->DMACTL = 0x00ul;
}
/**
...
__STATIC_INLINE void HSUSBD_SetEpBufAddr(uint32_t u32Ep, uint32_t u32Base, uint32_t u32Len)
{
    ...
    if (u32Ep == CEP)
    {
        pHSUSBD->CEPBUFST = u32Base;
        pHSUSBD->CEPBUFEND   = u32Base + u32Len - 1ul;
    }
    else
    {
        pHSUSBD->EP[u32Ep].EPBUFST = u32Base;
        pHSUSBD->EP[u32Ep].EPBUFEND = u32Base + u32Len - 1ul;
    }
}
...
__STATIC_INLINE void HSUSBD_ConfigEp(uint32_t u32Ep, uint32_t u32EpNum, uint32_t u32EpType, uint32_t u32EpDir)
{
    ...
    {
        pHSUSBD->EP[u32Ep].EPRSPCTL = (HSUSBD_EP_RSPCTL_FLUSH|HSUSBD_EP_RSPCTL_MODE_AUTO);
    }
    else if (u32EpType == HSUSBD_EP_CFG_TYPE_INT)
    {
        pHSUSBD->EP[u32Ep].EPRSPCTL = (HSUSBD_EP_RSPCTL_FLUSH|HSUSBD_EP_RSPCTL_MODE_MANUAL);
    }
    else if (u32EpType == HSUSBD_EP_CFG_TYPE_ISO)
    {
        pHSUSBD->EP[u32Ep].EPRSPCTL = (HSUSBD_EP_RSPCTL_FLUSH|HSUSBD_EP_RSPCTL_MODE_FLY);
    }

    pHSUSBD->EP[u32Ep].EPCFG = (u32EpType|u32EpDir|HSUSBD_EP_CFG_VALID|(u32EpNum << 4));
}
...
__STATIC_INLINE void HSUSBD_SetEpStall(uint32_t u32Ep)
{
    ...
    {
        pHSUSBD->EP[u32Ep].EPRSPCTL = (pHSUSBD->EP[u32Ep].EPRSPCTL & 0xf7ul) | HSUSBD_EP_RSPCTL_HALT;
    }
}
...
__STATIC_INLINE void HSUSBD_SetStall(uint32_t u32EpNum)
{
    ...
    else
    {
        ...
        {
            if (((pHSUSBD->EP[i].EPCFG & 0xf0ul) >> 4) == u32EpNum)
            {
                pHSUSBD->EP[i].EPRSPCTL = (pHSUSBD->EP[i].EPRSPCTL & 0xf7ul) | HSUSBD_EP_RSPCTL_HALT;
            }
        }
    }
}
...
__STATIC_INLINE void  HSUSBD_ClearEpStall(uint32_t u32Ep)
{
    ...
    pHSUSBD->EP[u32Ep].EPRSPCTL = HSUSBD_EP_RSPCTL_TOGGLE;
}
...
__STATIC_INLINE uint32_t HSUSBD_GetEpStall(uint32_t u32Ep)
{
    ...
    return (pHSUSBD->EP[u32Ep].EPRSPCTL & HSUSBD_EP_RSPCTL_HALT);
}
...
__STATIC_INLINE uint32_t HSUSBD_GetStall(uint32_t u32EpNum)
{
    ...
    for (i=0ul; i<HSUSBD_MAX_EP; i++)
    {
        if (((pHSUSBD->EP[i].EPCFG & 0xf0ul) >> 4) == u32EpNum)
        {
            val = (pHSUSBD->EP[i].EPRSPCTL & HSUSBD_EP_RSPCTL_HALT);
            break;
        }
    }
    return val;
