...
/*---------------------------------------------------------------------------------------------------------*/
/*  RTC LXT Gain Option Constant Definitions                                                               */
/*---------------------------------------------------------------------------------------------------------*/
#define RTC_LXT_GAIN_L0         (0UL)   /*!< Set LXT Gain to L0 mode \hideinitializer */
#define RTC_LXT_GAIN_L1         (1UL)   /*!< Set LXT Gain to L1 mode \hideinitializer */
#define RTC_LXT_GAIN_L2         (2UL)   /*!< Set LXT Gain to L2 mode \hideinitializer */
#define RTC_LXT_GAIN_L3         (3UL)   /*!< Set LXT Gain to L3 mode \hideinitializer */
...
/**
  * @brief      Get LXT Gain Mode Option
  *
  * @param      None
  *
  * @retval     0~3     LXT Gain mode option
  *
  * @details    This macro indicates LXT Gain mode setting.
  * \hideinitializer
  */
#define RTC_LXT_GAIN_MODE()                 ((RTC->LXTCTL & RTC_LXTCTL_GAIN_Msk) >> RTC_LXTCTL_GAIN_Pos)


/* Declare these inline functions here to avoid MISRA C 2004 rule 8.1 error */
__STATIC_INLINE void RTC_LXTGainInit(void);
__STATIC_INLINE void RTC_LXTGainSelection(uint32_t u32GainOption);

/**
  * @brief      Initial LXT Gain Mode
  *
  * @param      None
  *
  * @return     None
  *
  * @details    This function is used to initial the LXT Gain setting.
  */
__STATIC_INLINE void RTC_LXTGainInit(void)
{
    uint8_t u8IsRTCCKEN = 0;
    ...
    RTC_LXTGainSelection(RTC_LXT_GAIN_MODE());
...
/**
  * @brief      Select LXT Gain Option
  *
  * @param[in]  u32GainOption           Specify the LXT Gain option. It could be
  *                                     - \ref RTC_LXT_GAIN_L0
  *                                     - \ref RTC_LXT_GAIN_L1
  *                                     - \ref RTC_LXT_GAIN_L2
  *                                     - \ref RTC_LXT_GAIN_L3
  *
  * @details    This function is used to select the LXT Gain option.
  */
__STATIC_INLINE void RTC_LXTGainSelection(uint32_t u32GainOption)
{
    uint16_t au16TurboOnTbl[] = {15, 45, 55, 70};
    uint16_t au16TurboOffTbl[] = {9, 23, 34, 46};

    if(u32GainOption <= RTC_LXT_GAIN_L3)
    {
        RTC->LXTCTL = ((RTC->LXTCTL & ~(RTC_LXTCTL_GAIN_Msk | (0xFFFFul<<16))) | BIT23 |
                        (au16TurboOnTbl[u32GainOption] << 24) |
                        (au16TurboOffTbl[u32GainOption] << 16) |
                        (u32GainOption << RTC_LXTCTL_GAIN_Pos));
    }
}
