...
void PDMA_SetTransferMode(PDMA_T * pdma, uint32_t u32Ch, uint32_t u32Peripheral, uint32_t u32ScatterEn, uint32_t u32DescAddr)
{
    ...
    reg[u32Ch / 4] = (reg[u32Ch / 4] & ~(PDMA_REQSEL0_3_REQSRC0_Msk << ((u32Ch % 4) * 8))) | (u32Peripheral << ((u32Ch % 4) * 8));
...
{
    __IO uint32_t *reg = &((pdma)->TOC0_1);

    if(u32Ch >= PDMA_CH_MAX)
        return;

    /* Set timeout for the channel */
    reg[u32Ch / 2] = (reg[u32Ch / 2] & ~(PDMA_TOC0_1_TOC0_Msk << ((u32Ch % 2) * 16))) | (u32TimeOutCnt << ((u32Ch % 2) * 16));
...
void PDMA_Trigger(PDMA_T * pdma, uint32_t u32Ch)
{
    ...
    /* Get peripheral of the channel */
    u32Sel = (reg[u32Ch / 4] >> ((u32Ch % 4) * 8)) & PDMA_REQSEL0_3_REQSRC0_Msk;

    /* Software trigger only valid for memory to memory transfer */
    if(u32Sel == PDMA_MEM)
    {
