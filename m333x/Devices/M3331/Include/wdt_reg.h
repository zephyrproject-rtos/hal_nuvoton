...
typedef struct
{
     ...
     * |        |          |If this bit is set to 1, while WDT time-out interrupt flag IF (WDT_WDT_STATUS[0]) is generated to 1 and interrupt enable bit INTEN (WDT_CTL[6]) is enabled, the WDT time-out interrupt signal will generate a wake-up trigger event to chip.
     ...
     * |        |          |Note1: This bit also related to the WDT hardware enable function.
     * |        |          |CWDTEN[2:0] is CONFIG0[31][4][3],
     * |        |          |111 = WDT0 hardware enable function is inactive. User can set this bit to 0 or 1.
     * |        |          |110 = WDT0 hardware enable function is active. WDTPDIS be forced as 0 and user cannot change this bit to 1.
     * |        |          |Others = WDT0 hardware enable function is active. WDTPDIS be set as 0 and user can change this bit to 1.
     * |        |          |Note2: Only WDT0 supports hardware enable function.
